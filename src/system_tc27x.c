...
#include "C:\HighTec\toolchains\tricore\v4.9.3.0-infineon-1.0\tricore\include\tc27xd\IfxScu_reg.h"
...
/* check for devices with different clock control behaviour */
#if (RUN_ON_TC27XA == 1)
# define USE_OLD_CLOCK_SCHEME	1
#else
# define USE_OLD_CLOCK_SCHEME	0
#endif /* RUN_ON_TC27XA == 1 */
...
#if (USE_OLD_CLOCK_SCHEME == 1)
	0x0007001C, 0x01017600, 0x00020005, 0x12220101, 0x10012211, 0x00000202, 2
#else
	0x0007001C, 0x01017600, 0x00020505, 0x12120112, 0x10012242, 0x00000002, 2
#endif /* USE_OLD_CLOCK_SCHEME == 1 */
};
...
static const PllInitValue_t g_PllInitValue_100_50 =
{
	/* OSCCON,	PLLCON0,	PLLCON1,	CCUCON0,	CCUCON1,	CCUCON2,    finalK */
#if (USE_OLD_CLOCK_SCHEME == 1)
	0x0007001C, 0x01018a00, 0x00020006, 0x12220101, 0x10012211, 0x00000202, 6
#else
